...
`isAdmin()`은 "관리 화면을 열 수 있는가"이고, `isOwnerAccount()`는 "이 앱 주인의
계정인가"다. 주인은 `관_리자`와 `김_태우` 둘이고, 가입한 다른 사용자는 아니다.

홈 화면이 이 축으로 갈린다. 목표·퀘스트·미니메모·단위목표 카드는 주인의 개인
기능이고, 일반 사용자는 토마토 카드 중심의 축소된 홈을 받는다. 그 경로
(`renderTomatoCard()`)는 카드 하나를 더하는 게 아니라 **레이아웃을 재배치한다** —
단위목표 카드를 숨기고 식사·체중·생활존 카드를 걷어낸다.

계정을 나누기 전에는 이 구분도 `isAdmin()`이 겸했고, 모드 기본값이 `'admin'`이라
김_태우는 주인 홈을 보고 있었다. 권한만 계정으로 옮기고 이 축을 두지 않으면
개인 계정이 자기 카드를 잃는다. 화면 구성 분기에는 `isAdmin()`이 아니라
`isOwnerAccount()`를 쓴다.
