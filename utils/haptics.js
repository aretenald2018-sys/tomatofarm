...
// 세트 무게/횟수 입력용 짧은 틱. 10ms 이하는 selection(가장 약함),
// 그보다 크면 light로 보낸다 — 네이티브(Capacitor)와 설정 토글(_settingsEnabled)을
// 기존 _trigger 경로 그대로 태우고, 웹은 navigator.vibrate로 약하게 울린다.
export function hapticTick(durationMs = 8) {
  const ms = Math.max(1, Math.min(50, Math.floor(Number(durationMs)) || 8));
  void _trigger(ms <= 10 ? 'selection' : 'light');
}
