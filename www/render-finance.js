...
function _renderBudget() {
  ...
  let html = `<div style="overflow-x:auto"><table class="fin-budget-tbl">
    <thead><tr>
      ...
      <th class="bud-num">${_shortYear(_budgetYear)}'${_budgetQ}Q 목표</th>
  ...
  let grandTarget = 0, grandQGoal = 0, grandMonths = [0,0,0], grandQSum = 0;

  budgetDoc.groups.forEach((grp, gi) => {
    let grpTarget = 0, grpQGoal = 0, grpMonths = [0,0,0], grpQSum = 0;
    ...
    (grp.items || []).forEach((item, ii) => {
      ...
      const qGoal = (item.qGoals && item.qGoals[_budgetQ]) || 0;
      ...
      const overBudget = qGoal > 0 && qSum > qGoal;
      ...
      html += `<tr class="bud-item-row">
        ...
        <td class="bud-num bud-qgoal" onclick="editBudgetQGoal(${gi},${ii})">${_fmtBudget(qGoal)}</td>
        ...
        <td class="bud-num bud-qsum${overBudget ? ' bud-over' : ''}">${_fmtBudget(qSum)}</td>
      </tr>`;
    });

    // 그룹 소계
    const grpOver = grpQGoal > 0 && grpQSum > grpQGoal;
    html += `<tr class="bud-sum-row">
      <td class="bud-name"><b>소계</b></td>
      <td class="bud-num"><b>${_fmtBudget(grpTarget)}</b></td>
      <td class="bud-num"><b>${_fmtBudget(grpQGoal)}</b></td>
      ...
      <td class="bud-num${grpOver ? ' bud-over' : ''}"><b>${_fmtBudget(grpQSum)}</b></td>
    </tr>`;

    grandTarget += grpTarget;
    grandQGoal += grpQGoal;
    grandMonths[0] += grpMonths[0];
    grandMonths[1] += grpMonths[1];
    grandMonths[2] += grpMonths[2];
    grandQSum += grpQSum;
  });

  // TOTAL 행
  const totalOver = grandQGoal > 0 && grandQSum > grandQGoal;
  html += `<tr class="bud-total-row">
    <td class="bud-name"><b>TOTAL</b></td>
    <td class="bud-num"><b>${_fmtBudget(grandTarget)}</b></td>
    <td class="bud-num"><b>${_fmtBudget(grandQGoal)}</b></td>
    ...
    <td class="bud-num${totalOver ? ' bud-over' : ''}"><b>${_fmtBudget(grandQSum)}</b></td>
  </tr>`;
